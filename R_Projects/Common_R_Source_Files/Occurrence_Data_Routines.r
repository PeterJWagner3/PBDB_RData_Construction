...
taxon_collections <- name_unnamed_rock_units(paleodb_collections=taxon_collections,finest_chronostrat);
unique_rocks <- sort(unique(taxon_collections$rock_no_sr[taxon_collections$rock_no_sr>0]));
rock_finds <- data.frame(array(0,dim=c(length(unique_rocks),nbins)));
colnames(rock_finds) <- finest_chronostrat$interval;
# get rock-units
rn <- 0;
while (rn < length(unique_rocks))	{
...
accersi_strat_ranges_from_bin_by_taxa_matrix <- function(find_matrix,bins=c(),taxa=c())	{
...
ntaxa <- nrow(find_matrix);
if (is.null(taxa))	taxa <- rownames(find_matrix);
nbins <- ncol(find_matrix);
if (is.null(bins))	bins <- 1:nbins;
#tx <- taxa;
#ranges <- sapply(tx=taxa,extract_taxon_range_from_bin_by_taxa_matrix,find_matrix,bins)
ranges <- data.frame(FA=as.numeric(rep(0,ntaxa)),
										 LA=as.numeric(rep(0,ntaxa)));
rownames(ranges) <- taxa
for (tx in 1:ntaxa)
	ranges[tx,] <- extract_taxon_range_from_bin_by_taxa_matrix(tx,find_matrix,bins);
colnames(ranges) <- c("FA","LA")
return(ranges)
}

### ROUTINE TO GET BIN RANGES FOR A TAXON FROM A VECTOR OF 0's for absence, >0 for present
extract_taxon_range_from_bin_by_taxa_matrix	<- function(tx,find_matrix,bins=c())	{
if (is.null(bins))	bins <- 1:nrow(find_matrix);
return(c(min(bins[find_matrix[,tx]>=0.5]),max(bins[find_matrix[,tx]>=0.5])))
}

accersi_strat_ranges_from_taxa_by_bin_matrix <- function(find_matrix,bins=c(),taxa=c())	{
#find_matrix: bin x taxon matrix giving # occurrences per taxon per bin
#bins: array giving bin numbers corresponding to rows
#taxa: array giving taxon numbers corresponding to columns
ntaxa <- nrow(find_matrix);
...
ranges <- data.frame(FA=as.numeric(rep(0,ntaxa)),
										 LA=as.numeric(rep(0,ntaxa)));
rownames(ranges) <- taxa
ranges <- base::t(pbapply::pbsapply(1:ntaxa,extract_taxon_range_from_taxa_by_bin_matrix,find_matrix));
for (tx in 1:ntaxa)
	ranges[tx,] <- extract_taxon_range_from_taxa_by_bin_matrix(tx,find_matrix,bins);
colnames(ranges) <- c("FA","LA")
return(ranges)
}

extract_taxon_range_from_taxa_by_bin_matrix	<- function(tx,find_matrix,bins=c())	{
if (is.null(bins))	bins <- 1:ncol(find_matrix);
return(c(min(bins[find_matrix[tx,]>=0.5]),max(bins[find_matrix[tx,]>=0.5])))
}

# USE THIS TO GET STRAT RANGES FOR MANY TAXA AT ONCE!!!!
# modified 2024-01-12
accersi_taxon_ranges_from_taxa_by_bin_find_matrix	<- function(find_matrix,bins=c(),taxa=c())	{
if (is.null(bins))	bins <- 1:ncol(find_matrix);
if (is.null(taxa))	taxa <- rownames(find_matrix);
ntaxa <- nrow(find_matrix);
output <- data.frame(FA=as.numeric(rep(0,ntaxa)),LA=as.numeric(rep(0,ntaxa)));
rownames(output) <- rownames(find_matrix);
dummy <- pbapply::pbsapply(1:ntaxa,extract_taxon_range_from_taxa_by_bin_find_matrix,find_matrix);
output$FA <- simplify2array(dummy[1,]);
output$LA <- simplify2array(dummy[2,]);
return(output);
#return(c(min(bins[find_matrix[tx,]>=0.5]),max(bins[find_matrix[tx,]>=0.5])))
}

extract_taxon_range_from_taxa_by_bin_find_matrix	<- function(tx,find_matrix,bins=c())	{
if (is.null(bins))	bins <- 1:ncol(find_matrix);
output <- data.frame(FA=as.numeric(0),LA=as.numeric(0));
rownames(output) <- rownames(find_matrix)[tx];
if (max(find_matrix[tx,])<0.5)	{
	nbins <- ncol(find_matrix);
	going_up <- cumsum(find_matrix[tx,]);
	going_dn <- cumsum(find_matrix[tx,nbins:1])[nbins:1];
	going_an <- (going_up+going_dn)/2;
	find_matrix[tx,(1:nbins)[going_an %in% max(going_an)]] <- max(going_an);
	}
output$FA <- min(bins[find_matrix[tx,]>=0.5]);
output$LA <- max(bins[find_matrix[tx,]>=0.5]);
return(output);
#return(c(min(bins[find_matrix[tx,]>=0.5]),max(bins[find_matrix[tx,]>=0.5])))
}
...
#which(is.na(minimum_range_data),arr.ind=TRUE);
